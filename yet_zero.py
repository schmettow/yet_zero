## Simple eye tracking application for YET
...
def main():
    ...
    while True:
        ...
        if STATE == "Stimulus":
            elapsed_time = (
                time.time() - t_stim_started
            )  # elapsed time since STATE == "Stimulus" started
            if (
                elapsed_time > SLIDE_TIME
            ):  # presented longer then defined: trial is over
                Yet.data.to_csv(
                    RESULT_FILE, index=False
                )  ## auto save results after every slide
        ...
        if STATE == "Detect":
            if Yet.eye_detected:
                Img = yeti14.frame_to_surf(
                    Yet.eye_frame, (int(SURF_SIZE[0] * 0.5), int(SURF_SIZE[1] * 0.5))
                )
                draw_text("Eye detected!", SURF, (0.1, 0.85), FONT, color=col_green)
                draw_text("Space to continue", SURF, (0.1, 0.9), Font)
            else:
                Img = yeti14.frame_to_surf(
                    Yet.frame, (int(SURF_SIZE[0] * 0.5), int(SURF_SIZE[1] * 0.5))
                )
            ...
            Cal.draw()
            draw_text(
                "Follow the orange circle and press Space.", SURF, (0.1, 0.9), Font
            )
            """
            ...
            Yet.draw_follow(SURF)
            draw_text(
                "Look at the orange circle and press Space.", SURF, (0.05, 0.75), Font
            )
            """
...
def setup():
    ...
    global WD, STIM_DIR, STIM_PATH, RESULT_DIR, PART_ID, RESULT_FILE, EYECASC
